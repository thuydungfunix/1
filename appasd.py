...
def load_model():
    ...
    model.load_model("catboost_model.cbm")  # Đảm bảo file này có trong thư mục làm việc
    return model
...
def nhap_du_lieu():
    ...
    # Các đặc trưng khác (sắp xếp theo đúng thứ tự)
...
def du_doan(features):
    proba = model.predict_proba(features)[0][1]  # Xác suất mắc ASD
    pred = int(proba >= 0.5)  # Ngưỡng 0.5
    return pred, proba
